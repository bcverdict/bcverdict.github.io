const chartConfig = {
  ...
  options: {
    ...
    tooltips: {
      mode: 'nearest',
      intersect: false,
      backgroundColor: 'hsla(240,25%,76%,.81)',
      displayColors: false
    },
    legend: {
      display: false
    ...
    scales: {
      yAxes: [
        { display: true, scaleLabel: { display: true, labelString: 'Elo' } }
      ],
      xAxes: [
        {
          display: true,
          scaleLabel: { display: true, labelString: 'Opponents' },
          ticks: { display: false }
        }
      ]
    }
...
async function plot() {
      ...
      index == matches.length - 1
        ...
        : Math.round(
            (previousElo +=
              K *
              ((won ? 1 : 0) -
                1 / (1 + Math.pow(10, (opponent.elo - previousElo) / 400))))
          )
    )
  })
  document.getElementById('fullTitle').innerHTML = `All games`
  document.getElementById(
    'fullStats'
  ).innerHTML = `Wins: ${wins} Losses: ${matches.length - wins}`
  tableTop(fullTable)

  const rivalTable = document.getElementById('rivalTable')
  ...
  const rival = Array.from(frequency.keys())[
    frequencyValues.findIndex(value => value == maximumFrequency)
  ]

  document.getElementById('rivalTitle').innerHTML = `Rival: ${rival}`
  document.getElementById(
    'rivalStats'
  ).innerHTML = `Games played: ${maximumFrequency}`
  matches.forEach(match => {
    if (match.winning_algo.user != rival && match.losing_algo.user != rival)
      return
...
function insertMatch(table, match, algoId) {
  ...
  insertEntry(table, [
    ...
    match.turns,
    opponent.elo,
    `<a href='https://terminal.c1games.com/watch/${
      match.id
